...
public class BearLand2 {
		...
		try {
			...
			int a=Integer.parseInt(br.readLine());
				...
				{	//here we are dealing with each new test case
					...
					count=this.fixBears(zoo,0,0,bears,0);
					for (int i = 0; i < zoo.length; i++) {
	...
	public int fixBears(char[][] zoo,int idx,int jdx,char[]bears,int count)
	{	
		while(zoo[idx][jdx]!='?')
		{
		...
		if(!this.checkIsolatedQuestion(zoo, idx, jdx))
		{
			for (int i = 0; i < bears.length; i++) {
				...
				count+=this.fixBears(zoo,idx,jdx,bears,count);
